            ...
            while (NarrateLabel.waitFlag > 0 && running && client != null) {
                try {
                    ...
                    if (NarrateLabel.waitFlag <= 500) {
                        if (NarrateLabel.usingMouse.contains(usingString))
                            NarrateLabel.usingMouse = NarrateLabel.usingMouse.replace(usingString, "");
                        if (NarrateLabel.usingTab.contains(usingString))
                            NarrateLabel.usingTab = NarrateLabel.usingTab.replace(usingString, "");
                    }
